...
//#include<unistd.h> //for exit(0) function
...
int main(void)
{
	...
	gpFile=fopen("vzOglLog.txt","w");
	
	if(gpFile==NULL)
	{
		printf("ERROR: Unable to open file - vzOgl.txt");
		exit(-1);
	}
			...
			switch(event.type)
				{
					...
					case KeyPress:
						...
						//break;  //This break was not allowing to move down
						XLookupString(&event.xkey, ascii, sizeof(ascii),NULL, NULL);
						switch(ascii[0])
						{
							case 'E': // 'E'
								fprintf(gpFile,"INFO: 'E' Pressed \n");
								elbow = (elbow + 3) % 360;
								break;
							case 'e': // 'e'
								fprintf(gpFile,"INFO: 'e' Pressed \n");
...
void CreateWindow(void)
{
	...
	XStoreName(gpDisplay,gWindow, "Assignment 8 - RoboticArmWithLogFIle");
